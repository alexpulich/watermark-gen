		...
		$img.load(function(){
           ...
           .css({ width: "", height: "" });
    		...
    		if (width>height) {
    			...
    			$('.wraper__image-bg').css({'height':''+Math.round(height*653/width)+'px','width':'653px'});
    	   		} else {
    			$img.addClass('image-upload-h');
    			$('.wraper__image-bg').css({'height':'534px','width':''+Math.round(width*534/height)+'px'});
    		}
		});
 	...
 	$('#watermark').fileupload({
 		...
 		if ($('.wraper__image').length>0) {
			...
			// добавляем изображение в документ
	 		...
	 		$("#drag").draggable({ containment: ".image-upload", scroll: false });
 	}
